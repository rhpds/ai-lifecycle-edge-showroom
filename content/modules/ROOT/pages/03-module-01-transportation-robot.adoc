...
This module explores the complete infrastructure running on RedBot Logistics' autonomous transportation robots. Each robot runs **Red Hat Device Edge (MicroShift)**, a lightweight Kubernetes platform optimized for resource-constrained environments.
...
NOTE: If a message regarding the authenticity of the host is displayed, type `yes` to establish the connection.
...
NOTE: Press `Ctrl+C` to exit the log view.
...
MinIO provides S3-compatible object storage for AI models on the robot. The `inference` bucket stores two AI models:
...
In the terminal, type the following to create the default project for GitOps applications:
...
In the terminal, type the following to deploy MinIO by creating a GitOps Application:
...
. Open the MinIO dashboard and login with the provided credentials:
+
...
In the terminal, type the following to deploy the inference server GitOps application:
...
In the terminal, type the following to deploy the complete Battery Monitoring System application:
        ...
        - target:
          ...
          patch: |-
            - op: replace
              ...
              value: |
                {
                  "BATTERY_METRICS_WS_ENDPOINT": "wss://mqtt2ws-microshift-001.{ai_edge_apps_domain}/battery/metrics/",
                  "OPEN_AI_API_KEY": "my-key",
                  "STRESS_DETECTION_AI_API_ENDPOINT": "https://stress-detection-predictor-microshift-001.{ai_edge_apps_domain}/",
                  "TIME_TO_FAILURE_AI_API_ENDPOINT": "https://time-to-failure-predictor-microshift-001.{ai_edge_apps_domain}/",
                  "BATTERY_SIMULATION_API_ENDPOINT": "https://battery-simulation-microshift-001.{ai_edge_apps_domain}/",
...
. Open the InfluxDB dashboard and login using the credentials provided:
+
[subs="attributes,macros"]
----
...
* How to access edge infrastructure via the bastion host
...
Navigate to xref:04-module-02-rhoai-configuration.adoc[Module 2: Red Hat OpenShift AI configuration].
