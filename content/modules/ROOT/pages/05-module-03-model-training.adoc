...
[#review-workbench]
== Exercise 3.1: Review workbench configuration
...
* **Image selection**: `Jupyter | TensorFlow | CUDA | Python 3.12`
* **Version selection**: `2025.2` (latest version available)
* **Hardware profile**: `default-profile` (2 CPUs and 4 GiB of memory)

=== Attach DataConnection

Conect the Workbench to the existing DataConnection that contains the credentials to access the MinIO instance in the transportation robot.
...
3. **bms-stress-training.ipynb** - Trains the Stress Detection model and saves it localy
4. **predict-stress.ipynb** - Validates the Stres Detection model performance with new data
5. **bms-ttf-training.ipynb** - Trains the Time to Failure model and saves it localy
