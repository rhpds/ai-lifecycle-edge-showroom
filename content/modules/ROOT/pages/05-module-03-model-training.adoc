...
. Keep scrolling until you reach the *Connections* section. Here we will connect the Workbench to the existing DataConnection that contains the credentials to access the MinIO instance in the transportation robot
...
. Click **model-training** to access the model training JupyterLab environment
...
+
** **01_retrieve-influx-data.ipynb** - Retrieve battery telemetry from InfluxDB on robots
** **02_prepare-data.ipynb** - Clean, normalize, and prepare training datasets
** **03_bms-stress-training.ipynb** - Trains the Stress Detection model and saves it locally
** **04_predict-stress.ipynb** - Validates the Stress Detection model performance with new data
** **05_bms-ttf-training.ipynb** - Trains the Time to Failure model and saves it locally
** **06_predict-ttf.ipynb** - Validates the Time to Failure model performance with new data
** **07_save-models.ipynb** - Store the new models in the MicroShift's MinIO instance

. At this point, you can open each Notebook in order and follow the instructions contained. Pay attention to some of the variables used during the execution. You will need to replace in the first notebook the InfluxDB URL with yours:
+
[source,yaml,options="wrap",role="execute",subs="attributes+"]
----
...
---- 
+
. Execute each Notebook. You can do it by pressing the **Play** button at the top to manually run each cell or the **Double Play** icon to run the entire file
+
image::05-module-03_run-notebooks.png[Play and Double Play buttons to run the Notebook,link=self,window=blank,width=700]

Once you have run all of them, the new AI models should have been uploaded to the vehicle’s MinIO storage and will now be used by the Battery Monitoring System for real-time stress detection and time-to-failure predictions.

=== Verify model export

After all notebooks have been executed, check that models were exported to OpenVINO format and MinIO contains the updated models:
