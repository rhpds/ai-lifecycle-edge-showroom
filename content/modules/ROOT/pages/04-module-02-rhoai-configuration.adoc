...
. Navigate to the OpenShift AI tab or open the dashboard URL provided below and login with the OpenShift credentials:
+
...
. Locate and click on the `ai-edge-project` that has been pre-created
...
Data connections link Red Hat OpenShift AI to external object storage. The `vehicle-models` connection provides access to the robot's MinIO instance where the AI models are stored. This connection bridges the SNO training environment and robot's edge storage.
...
Data connections are stored as Kubernetes Secrets containing:
